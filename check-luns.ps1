...
$ESXHosts = Get-VMHost
$output = "{0},{1},{2},{3},{4},{6}`r`n" -f "Hostname", "Device Name", "VAAIPluginName", "ATSStatus", "CloneStatus", "DeleteStatus", "ZeroStatus"
foreach($EsxHost in $ESXHosts){
  ...
  foreach ($LUN in $esxclioutput){
    ...
    $output = "{0}{1},{2},{3},{4},{6},{7}`r`n" -f $output, $esxHost, $LUN.Device, $LUN.VAAIPluginName, $LUN.ATSStatus,$LUN.CloneStatus, $LUN.DeleteStatus, $LUN.ZeroStatus
  }
}
